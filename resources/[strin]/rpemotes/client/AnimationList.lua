...
local function GetAllAnimations()
    local allSections = {RP.Shared, RP.Dances, RP.AnimalEmotes, RP.Exits, RP.Emotes, RP.PropEmotes}
    local allAnimations = {}
    for _,section in pairs(allSections) do
        for emoteName, emote in pairs(section) do
            allAnimations[emoteName] = emote
        end
    end
...
exports("IsEntityPlayingAnyAnim", function(entity)
    local allAnimations = GetAllAnimations()
    local isEntityPlayingAnyAnim = false
    for _,v in pairs(allAnimations) do
        if(IsEntityPlayingAnim(entity, v[1], v[2], 3) or IsPedUsingAnyScenario(entity)) then
            isEntityPlayingAnyAnim = true
            break
        end
    end
    return isEntityPlayingAnyAnim
end)
