/*
    Díky "cui_character" za logiku s pohyblivou in-game kamerou v NUI. <3
    - Strin
*/ 
IsModelLoaded = false
...
Camera.Activate = function(delay)
    ...
    while true do
        Citizen.Wait(0)
        if(coords.z - groundZ <= 10 and IsModelLoaded and not IsPedFalling(playerPed)) then
            break
        end
    end
...
Camera.SetView = function(view)
    ...
    --print(Camera.entity, Camera.position.x, Camera.position.y, Camera.position.z)

    targetPos = GetPedBoneCoords(previewPed, boneIndex, 0.0, 0.0, 0.0)
    --print(Camera.entity, targetPos.x, targetPos.y, targetPos.z)
...
Citizen.CreateThread(function() 
    while true do
        local sleep = 1000
        if Camera.active or InMenu then
            sleep = 0
