...
RegisterKeyMapping('alert_gps', '<FONT FACE="Righteous">Nastavit waypoint~</FONT>', 'keyboard', 'Y')
				...
				if GetVehiclePedIsUsing(playerPed) ~= 0 then
					local vehicle = GetVehiclePedIsUsing(playerPed, true)
						...
						if Config.Timer['Shooting'] == 0 and not BlacklistedWeapon(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and IsPedArmed(playerPed, 4) then
							...
							if IsPedShooting(playerPed) and zoneChance('Driveby', 2, currentStreetName) then
								...
								data = {dispatchCode = 'driveby', caller = _U('caller_local'), coords = playerCoords, netId = veh.id, length = 11000,
						...
						elseif Config.Timer['Autotheft'] == 0 and (IsPedGettingIntoAVehicle(playerPed) and GetSeatPedIsTryingToEnter(playerPed) == -1) and ((driver > 0 and not IsPedAPlayer(driver)) or IsVehicleAlarmActivated(vehicle)) then
							sleep = 100
							local veh = vehicleData(vehicle)
							lib.callback('linden_outlawalert:isVehicleOwned', false, function(hasowner) veh.owned = hasowner end, veh.plate)
					...
					if Config.Timer['Shooting'] == 0 and not BlacklistedWeapon(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and IsPedArmed(playerPed, 4) then
						...
						if IsPedShooting(playerPed) and zoneChance('Shooting', 2, currentStreetName) then
							data = {dispatchCode = 'shooting', caller = _U('caller_local'), coords = playerCoords, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 11000}
							TriggerServerEvent('wf-alerts:svNotify', data)
							Config.Timer['Shooting'] = Config.Shooting.Success
						else
							Config.Timer['Shooting'] = Config.Shooting.Fail
					...
					elseif Config.Timer['Melee'] == 0 and IsPedInMeleeCombat(playerPed) and HasPedBeenDamagedByWeapon(GetMeleeTargetForPed(playerPed), 0, 1) then
						...
						if zoneChance('Melee', 3, currentStreetName) then
							data = {dispatchCode = 'melee', caller = _U('caller_local'), coords = playerCoords, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 11000}
							TriggerServerEvent('wf-alerts:svNotify', data)
...
RegisterCommand('alert_dead', function()
	if playerIsDead and canSendDistress then
		...
		if isPlayerWhitelisted then
			Citizen.Wait(2000)
			data = {dispatchCode = 'officerdown', caller = name, coords = GetEntityCoords(PlayerPedId()), netId = netId, info = title, length = 10000}
			TriggerServerEvent('wf-alerts:svNotify', data)
			TriggerEvent("strin_deathscreen:distress", true)
			TriggerServerEvent("strin_jobs:startDistress")
			Citizen.Wait(20000)
			canSendDistress = true
			TriggerEvent("strin_deathscreen:distress", false)
		elseif Config.Enable.PlayerDowned then
			Citizen.Wait(2000)
			data = {dispatchCode = 'persondown', _U('caller_local'), coords = GetEntityCoords(PlayerPedId()), netId = netId, length = 8000}
			TriggerServerEvent('wf-alerts:svNotify', data)
			TriggerEvent("strin_deathscreen:distress", true)
			TriggerServerEvent("strin_jobs:startDistress")
			Citizen.Wait(20000)
...
RegisterKeyMapping('alert_dead', '<FONT FACE="Righteous">Tísňové volání~</FONT>', 'keyboard', 'G')

RegisterCommand('911', function(playerId, args, rawCommand)
	if not args[1] then 
		TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text="Zpráva nesmí být prázdná!"})
		 return end
	args = table.concat(args, ' ')
	local caller
	if Config.PhoneNumber then caller = phone else caller = ('%s %s'):format(firstname, lastname) end
	if Config.Default911 then TriggerServerEvent('mdt:newCall', args, caller, GetEntityCoords(PlayerPedId())) else
		TriggerServerEvent('wf-alerts:svNotify911', args, caller, GetEntityCoords(PlayerPedId()))
	end
	TriggerEvent('mythic_notify:client:SendAlert', {type = "inform", text="Vaše zpráva byla zaslána příslušným jednotkám."})
end, false)

RegisterCommand('911m', function(playerId, args, rawCommand)
	if not args[1] then TriggerEvent('mythic_notify:client:SendAlert', {type = "error", text="Zpráva nesmí být prázdná!"}) return end
	args = table.concat(args, ' ')
	if Config.Default911 then TriggerServerEvent('mdt:newCall', args, _U('caller_unknown'), GetEntityCoords(PlayerPedId())) else
		TriggerServerEvent('wf-alerts:svNotify911', args, _U('caller_unknown'), GetEntityCoords(PlayerPedId()))
	end
	TriggerEvent('mythic_notify:client:SendAlert', {type = "inform", text="Vaše zpráva byla zaslána příslušným jednotkám."})
end, false)
