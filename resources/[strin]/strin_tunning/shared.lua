...
LoadLocale = function(lang)
    local localeFile = LoadResourceFile("strin_tunning", ("locales/%s.lua"):format(lang))
    local loadHandle = load(localeFile)
    Locales = loadHandle()
end
...
_U = function(entry, ...)
    return Locales[entry] and (Locales[entry]:format(...)):gsub("^%l", string.upper) or "Nedefinováno"
end

function GetColoursFromName(colour)
	local colours = {}
...
function CountKeysInTable(desiredTable)
    local keyCount = 0
