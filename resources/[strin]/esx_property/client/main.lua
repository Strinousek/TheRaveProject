...
function RefreshBlips()
  ...
  for k, v in pairs(Properties) do
    if v.Owned and Config.OwnedBlips then
      if v.Owner:find(ESX.PlayerData.identifier) then
        ...
        AddTextComponentString("<FONT FACE='Righteous'>"..v.Name.."</FONT>")
...
RegisterNetEvent("esx_property:syncProperties", function(properties, lastProperty)
  ...
  for house, data in pairs(Properties) do
    if data.Keys then
      for ident, vaues in pairs(data.Keys) do
        if vaues and ident:find(ESX.PlayerData.identifier) then
  ...
  if lastProperty then
    if Properties[lastProperty.id] and (Properties[lastProperty.id].Owner:find(ESX.PlayerData.identifier) or PlayerKeys[lastProperty.id]) then
...
exports("IsPlayerInProperty", function()
  return InProperty
end)

exports("GetPropertyPlayerIsIn", function()
  if(InProperty and CurrentId ~= 0) then
    return Properties[CurrentId], PlayerKeys[CurrentId]
  else
    return nil, nil
  end
end)
...
function PropertyMenuElements(PropertyId)
  ...
  if Property.Owned then
    ...
    if Property.Owner:find(ESX.PlayerData.identifier) then
    ...
    if not InProperty then
      ...
      if (Config.CCTV.Enabled and Properties[PropertyId].cctv.enabled) and (Property.Owner:find(ESX.PlayerData.identifier) or PlayerKeys[PropertyId]) then
      ...
      if (Property.Owner:find(ESX.PlayerData.identifier) or PlayerKeys[PropertyId]) then
    ...
    if (not Property.Locked or Config.OwnerCanAlwaysEnter and Property.Owner:find(ESX.PlayerData.identifier)) and not InProperty then
    ...
    if Property.Locked and not InProperty and
      not (Config.OwnerCanAlwaysEnter and Property.Owner:find(ESX.PlayerData.identifier) or PlayerKeys[PropertyId]) then
    ...
    if #(Properties) > 0 then
      ...
      for i = 1, #(Properties) do
        if Properties[i].Entrance then
          ...
          if Properties[i].garage.enabled and Properties[i].garage.pos and (Properties[i].Owner:find(ESX.PlayerData.identifier) or PlayerKeys[i]) then
