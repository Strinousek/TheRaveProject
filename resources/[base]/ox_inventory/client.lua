...
local function registerCommands()
	...
	local function openGlovebox(vehicle)
		...
		local vehicleIdentifier = Entity(vehicle).state.vehicleIdentifier or GetVehicleNumberPlateText(vehicle)
		local isOpen = client.openInventory('glovebox', { id = 'glove'..vehicleIdentifier, netid = NetworkGetNetworkIdFromEntity(vehicle) })
	...
	lib.addKeybind({
		...
		onPressed = function(self)
			...
			if #(playerCoords - position) < 3 and door then
				local plate = Entity(entity).state.vehicleIdentifier or GetVehicleNumberPlateText(entity)
