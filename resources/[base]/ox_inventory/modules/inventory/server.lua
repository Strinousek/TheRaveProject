...
local function loadInventoryData(data, player)
	...
	if data.type == 'trunk' or data.type == 'glovebox' then
		...
		local vehicleIdentifier = plate:len() == 12 and plate or nil
			...
			if data.netid then
				...
				for i = 1, #vehicles do
					...
					if(vehicleIdentifier) then
						local spawnedVehicle = exports.strin_garages:GetSpawnedVehicle("identifier", vehicleIdentifier)
						if(spawnedVehicle) then
							entity = vehicle
							data.netid = spawnedVehicle.netId
							break
						end
						goto skipVehicleLoop
					end
