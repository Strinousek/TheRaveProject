				...
				for(let i=0; i<menuData.rows.length; i++){
					...
					for(let j=0; j<row.cols.length; j++){
						...
						for(let k=0; k<matches.length; k++)
							col = col.replace('{{' + matches[k][1] + '|' + matches[k][2] + '}}', '<button class="button" data-id="' + i + '" data-namespace="' + namespace + '" data-name="' + name + '" data-value="' + matches[k][2] +'">' + matches[k][1] + '</button>');
