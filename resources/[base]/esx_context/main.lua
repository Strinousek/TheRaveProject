...
function Open(position, eles, onSelect, onClose, canClose)
	...
	local validatedElements = {}
	for i=1, #eles do
		local ele = eles[i]
		validatedElements[i] = {
			disabled = ele.unselectable or false,
			description = ele.description or nil,
			title = ele.inputValue and (ele.title..": "..ele.inputValue) or ele.title,
			icon = ele.icon or nil,
			onSelect = function()
				if(ele.input) then
					ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "dialog_"..MenuId, {
						title = ele.inputPlaceholder or "Hodnota",
					}, function(data, menu)
						eles[i].inputValue = ele.inputType == "text" and (data.value or "") or (tonumber(data.value) or 0)
						menu.close()
						Open(position, eles, onSelect, onClose, canClose)
	...
	lib.registerContext({
		id = "esx-context:"..MenuId,
		title = "Kontextové Menu",
		canClose = canClose,
		onExit = function()
			Closed(true)
		end,
		options = validatedElements
	})
	lib.showContext("esx-context:"..MenuId)
	MenuId += 1
end

function Closed(experiment)
	--SetNuiFocus(false, false)
	if(lib.getOpenContextMenu() and not experiment) then
...
exports("Refresh", function(eles, position)
	...
	Open(activeMenu.position, activeMenu.eles, activeMenu.onSelect, activeMenu.onClose, activeMenu.canClose)
