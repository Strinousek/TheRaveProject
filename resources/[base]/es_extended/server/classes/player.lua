...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
	...
	function self.setJob(job, grade)
		...
		local lastJob = lib.table.deepclone(self.job)
		if(grade == 0) then
			print(("%s tried job %s for grade - 0"):format(self.identifier, job))
			grade = 1
		end
		...
		if ESX.DoesJobExist(job, grade) then
			...
			if gradeObject.skin_male then
				self.job.skin_male = type(gradeObject.skin_male) == "string" and json.decode(gradeObject.skin_male) or gradeObject.skin_male
			else
				self.job.skin_male = {}
			end

			if gradeObject.skin_female then
				self.job.skin_female = type(gradeObject.skin_female) == "string" and json.decode(gradeObject.skin_female) or gradeObject.skin_female
			else
	...
	function self.showNotification(msg, props)
		self.triggerEvent('esx:showNotification', msg, props or nil)
	end
