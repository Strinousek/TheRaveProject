...
if Config.Identity then
    loadPlayer = loadPlayer .. ', `firstname`, `lastname`, `dateofbirth`, `sex`, `height`, `char_id`'
end
...
function loadESXPlayer(identifier, playerId, isNew)
  ...
  local result = MySQL.single.await(loadPlayer, { identifier })
  local job, grade, jobObject, gradeObject = result.job, result.job_grade
  ...
  if result.skin and result.skin ~= '' then
    ...
    if userData.sex == 'f' or userData.sex == "F" then
      userData.skin = {sex = 1}
    else
      userData.skin = {sex = 0}
    end
  ...
  if result.firstname and result.firstname ~= '' then
    ...
    if result.char_id then
        userData.char_id = result.char_id
    end
  ...
  if userData.firstname then
    ...
    if userData.char_id then
      xPlayer.set('char_id', userData.char_id)
    end
  ...
  xPlayer.triggerEvent('esx:playerLoaded',
    {
      ...
      fullname = xPlayer.get("fullname") or "John Doe",
