...
function ESX.RefreshJobs()
  ...
  for _, v in ipairs(jobs) do
    Jobs[v.name] = {
      name = v.name,
      label = v.label, -- Add Event Handler for strin_society to keep this up to date
      balance = v.balance, -- Add Event Handler for strin_society to keep this up to date
      grades = {}
    }
  end
  ...
  for _, v in ipairs(jobGrades) do
    if Jobs[v.job_name] then
      Jobs[v.job_name].grades[v.grade] = {
        label = v.label,
        grade = v.grade,
        name = v.name,
        salary = v.salary,
        skin_male = {}, -- compatibility shit
        skin_female = {}, -- compatibility shit
      }
    else
...
AddEventHandler("strin_society:societyChange", function(society)
    local job = ESX.Jobs[society.name]
    if(job.label ~= society.label) then
      ESX.Jobs[society.name].label = society.label
    end
    if(job.balance ~= society.balance) then
      ESX.Jobs[society.name].balance = society.balance
    end
    for k,v in pairs(society.grades) do
      local grade = job.grades[k]
      local societyGrade = society.grades[k]
      if(grade and not societyGrade) then
        ESX.Jobs[society.name].grades[k] = nil
      end
      if(not grade) then
        ESX.Jobs[society.name].grades[k] = v
      end
      if(grade.name ~= societyGrade.name) then
        ESX.Jobs[society.name].grades[k].name = societyGrade.name
      end
      if(grade.label ~= societyGrade.label) then
        ESX.Jobs[society.name].grades[k].label = societyGrade.label
      end
      if(grade.salary ~= societyGrade.salary) then
        ESX.Jobs[society.name].grades[k].salary = societyGrade.salary
      end
    end
end)

AddEventHandler("strin_society:societyCreated", function(society)
  local job = ESX.Jobs[society.name]
  if(not job) then
      local newJob = {}
      for k,v in pairs(society) do
        if(not k:find("name")) then
          newJob[k] = v
        end
      end
      for i=1, #newJob.grades do
        newJob.grades[i].skin_female = {}
        newJob.grades[i].skin_male = {}
      end
      ESX.Jobs[society.name] = newJob
  end
end)
