...
function ESX.ShowNotification(msg, properties)
    if(type(properties) == "string") then
        properties = { type = properties }
    end
	local settings = properties or {}
	if(next(settings) == nil) then
		TriggerEvent("strin_notify:notify", { type = "inform", text = msg })
	else
		local type = "inform"
		local length = 4000
		...
		if(settings.length or settings.time or settings.duration) then
			if(settings.time) then
				length = settings.time
			elseif(settings.length) then
				length = settings.length
            elseif(settings.duration) then
                length = settings.duration
			end
		end
		TriggerEvent("strin_notify:custom_notify", {type = type, text = msg, length = length})
	end
end
...
function ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
	TriggerEvent("strin_notify:notify", {type = "inform", text = '<b>'.. sender .. '</b> - '.. subject ..'<br>' .. msg})
end

function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
	local msg = "<FONT FACE='Righteous'>" .. msg
	msg = string.gsub(msg, '~INPUT_PICKUP~', "~g~[E]~w~")
	msg = string.gsub(msg, '~INPUT_CONTEXT~', "~g~[E]~w~")
	msg = string.gsub(msg, '~INPUT_DETONATE~', "~g~[G]~w~")
...
AddEventHandler('esx:showNotification', function(msg, props)
	ESX.ShowNotification(msg, props)
end)
