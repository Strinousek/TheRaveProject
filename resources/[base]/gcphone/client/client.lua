...
local KeyToucheCloseEvent = {
  { code = 172, event = 'ArrowUp', label = "Pohyb nahoru" },
  { code = 173, event = 'ArrowDown', label = "Pohyb dolu" },
  { code = 174, event = 'ArrowLeft', label = "Pohyb vlevo" },
  { code = 175, event = 'ArrowRight', label = "Pohyb vpravo" },
  { code = 176, event = 'Enter', label = "Potvrdit" },
  { code = 177, event = 'Backspace', label = "Zpět" },
}
...
function hasPhone (cb)
  local count = exports.ox_inventory:GetItemCount("phone") or 0
  cb(count > 0)
end
--====================================================================================
...
function ShowNoPhoneWarning ()
    ESX.ShowNotification("Nemáte mobil!", {type = "error"})
end
...
RegisterCommand("phone", function()
    if(menuIsOpen or isDead) then
        if(menuIsOpen and not isDead) then
          TooglePhone()
          return
        end
        ESX.ShowNotification("Nemůžete otevřít mobil!", { type = "error" })
        return
    end
...
RegisterKeyMapping("phone", "<FONT FACE='Righteous'>Mobil~</FONT>", "keyboard", "F1")

do
    for _,v in pairs(KeyToucheCloseEvent) do
        local commandName = "phone"..v.event:lower()
        RegisterCommand(commandName, function()
            if(menuIsOpen) then
                SendNUIMessage({keyUp = v.event})
            end
        end)

        local arrowIndex = v.event:find("Arrow")
        local key = arrowIndex and v.event:sub(arrowIndex + ("Arrow"):len()):upper() or v.event:upper()
        key = (key == "BACKSPACE") and "BACK" or ((key == "ENTER") and "RETURN" or key)
        RegisterKeyMapping(commandName, "<FONT FACE='Righteous'>Mobil - "..v.label.."~</FONT>", "keyboard", key)
    end
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if takePhoto ~= true then
            local sleep = 0
            if menuIsOpen == true and useMouse then
...
function startFixeCall (fixeNumber)
  ...
  ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "gcphone_call", {
    label = "Číslo"
  }, function(data, menu)
    number = data.value or ""
    menu.close()
...
RegisterNUICallback('reponseText', function(data, cb)
  ...
  ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "gcphone_message", {
    label = "Zpráva"
  }, function(data, menu)
    text = data.value or ""
    menu.close()
